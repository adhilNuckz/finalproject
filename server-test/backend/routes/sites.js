  ...
  exec(`[ -L ${enabledFile} ] && echo "enabled" || echo "disabled"`, (err, stdout) => {
    ...
    if (action === 'enable') {
      ...
      if (hasBackup) {
        ...
        const maintenanceHTML = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Under Maintenance</title><meta name="viewport" content="width=device-width,initial-scale=1.0"><style>*{margin:0;padding:0;box-sizing:border-box;font-family:Segoe UI,Arial}body{height:100vh;display:flex;align-items:center;justify-content:center;background:radial-gradient(circle at top,#1b1b1b,#0b0b0b);color:#fff}div{background:#141414;padding:45px;border-radius:14px;text-align:center;width:360px;box-shadow:0 0 40px rgba(0,0,0,.8)}h1{font-size:26px;margin:15px 0;color:#00ffd5}p{color:#aaa;margin-bottom:25px;line-height:1.5}.gear{font-size:64px;animation:spin 3s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.pulse{display:flex;justify-content:center;gap:10px}.pulse span{width:12px;height:12px;background:#00ffd5;border-radius:50%;animation:pulse 1.4s infinite ease-in-out}.pulse span:nth-child(2){animation-delay:.2s}.pulse span:nth-child(3){animation-delay:.4s}@keyframes pulse{0%,100%{opacity:.3;transform:scale(.8)}50%{opacity:1;transform:scale(1.2)}}footer{margin-top:20px;font-size:12px;color:#555}</style></head><body><div><div class="gear">⚙️</div><h1>Under Maintenance</h1><p>We are currently working on improvements.<br>Please check back shortly.</p><div class="pulse"><span></span><span></span><span></span></div><footer>© 2025 • System Status</footer></div></body></html>';
